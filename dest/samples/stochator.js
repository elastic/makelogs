...
(function () {
  var Set,
      ...
      randomCharacter,
      randomColor,
      randomNormallyDistributedFloat,
      randomSetMember,
  ...
  callFunctions = function callFunctions(fns) {
  ...
  randomBoundedFloat = function randomBoundedFloat(min, max) {
    var spread;
    if (min == null) min = 0;
    if (max == null) max = 1;
    spread = max - min;
  ...
  randomBoundedInteger = function randomBoundedInteger(min, max) {
    var spread;
    if (min == null) min = 0;
    if (max == null) max = 1;
    spread = 1 + max - min;
  ...
  randomColor = function randomColor() {
    ...
    mutator = function mutator(bytes) {
      var blue, green, red;
  ...
  randomNormallyDistributedFloat = function randomNormallyDistributedFloat(mean, stdev, min, max) {
    var float, seed;
  ...
  randomCharacter = function randomCharacter(lowercase) {
    var max, min, mutator, _ref;
    _ref = lowercase ? [97, 122] : [65, 90], min = _ref[0], max = _ref[1];
    mutator = function mutator(charCode) {
  ...
  randomSetMember = function randomSetMember(set) {
    var max;
    max = set.length - 1;
    return set.get(randomBoundedInteger(0, max));
  };

  randomSetMemberWithoutReplacement = function randomSetMemberWithoutReplacement(set) {
    if (!set.get(0)) return;
    set.length -= 1;
    return set.pop(randomBoundedInteger(0, set.length));
  };

  randomWeightedSetMember = function randomWeightedSetMember(set, weights) {
  ...
  inverseNormalCumulativeDistribution = function inverseNormalCumulativeDistribution(probability) {
    ...
    mapMaxExp = function mapMaxExp(maxExp) {
  ...
  shuffleSet = function shuffleSet(set) {
  ...
  floatGenerator = function floatGenerator(min, max, mean, stdev) {
    if (mean && stdev) {
      return function () {
  ...
  integerGenerator = function integerGenerator(min, max) {
    if (min == null) min = 0;
    if (max == null) max = 1;
    return function () {
  ...
  setGenerator = function setGenerator(values, replacement, shuffle, weights) {
    var set, weightsSet;
  ...
  Stochator = function () {
    ...
    Stochator.prototype.createGenerators = function (configs, mutator) {
      ...
      if (!mutator) {
        ...
        callGenerators = function callGenerators() {
...
//# sourceMappingURL=stochator.js.map
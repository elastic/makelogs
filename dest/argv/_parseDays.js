...
module.exports = function parseDays(argv) {
  ...
  if (_.isNumber(argv.days)) {
    return [startBase.subtract(argv.days, 'days'), endBase.add(argv.days, 'days')];
  }

  if (_.isString(argv.days)) {
    if (customDayBoundsRE.test(argv.days)) {
      var ends = argv.days.split('/').map(parseFloat);
      return [startBase.subtract(ends[0], 'days'), endBase.add(ends[1], 'days')];
...
//# sourceMappingURL=_parseDays.js.map